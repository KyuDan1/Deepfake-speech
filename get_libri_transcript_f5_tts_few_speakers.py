...
OUTPUT_DIR = Path("generated_results_F5_FewSpeakers").resolve()
PROMPT_WAV_DIR = Path("prompt_wav_files").resolve()
FIGURE_DIR = Path("analysis_figures_f5_tts_few_speakers").resolve()
...
if generation_queue:
    ...
    print("모든 오디오 파일이 이미 존재합니다.")


# ==========================================
# 3. 시각화 함수 (2-Subplot 버전)
# ==========================================

def plot_dual_subplot(X_embedded, y, filenames, speaker_ids, title, filename_suffix):
    """
    2개의 subplot으로 시각화:
    - 왼쪽: Real(파랑) vs Gen(초록) 기본 비교
    - 오른쪽: Speaker별 색상 구분 (Real/Gen은 명도 차이)
    """
    ...
    ax1.scatter(X_embedded[mask_gen, 0], X_embedded[mask_gen, 1],
               c='green', label='Gen (F5-TTS)', marker='^',
               s=80, alpha=0.7, edgecolors='white', zorder=2)
    ...
    # Speaker별 색상 (Real/Gen 구분)
    speaker_colors = {
        (unique_speakers[0], 0): 'dodgerblue',     # Speaker1 Real
        (unique_speakers[0], 1): 'navy',           # Speaker1 Gen
    }
    speaker_labels = {
    ...
    if len(unique_speakers) >= 2:
        speaker_colors[(unique_speakers[1], 0)] = 'lightcoral'   # Speaker2 Real
        speaker_colors[(unique_speakers[1], 1)] = 'darkred'      # Speaker2 Gen
...
def plot_dual_by_gender(X_embedded, y, filenames, speaker_ids, title, filename_suffix):
    """
    2개의 subplot으로 시각화 (Gender 버전):
    - 왼쪽: Real(파랑) vs Gen(초록)
    - 오른쪽: Gender별 + Real/Gen별 색상 구분
    """
    fig, axes = plt.subplots(1, 2, figsize=(20, 9))
    ...
    color_map = {
        ('M', 0): 'dodgerblue',      # Male Real
        ('M', 1): 'navy',            # Male Gen
        ('F', 0): 'lightcoral',      # Female Real
        ('F', 1): 'darkred',         # Female Gen
    }
    label_map = {
        ('M', 0): 'Male (Real)',
        ('M', 1): 'Male (Gen)',
...
plot_dual_subplot(X_tsne, y_grad, filenames, speaker_ids, "t-SNE (Gradient Field)", "Gradient_tSNE")
plot_dual_by_gender(X_tsne, y_grad, filenames, speaker_ids, "t-SNE (Gradient Field)", "Gradient_tSNE")

reducer = umap.UMAP(n_neighbors=min(15, len(X_grad)-1), min_dist=0.3, random_state=42)
X_umap = reducer.fit_transform(X_grad_scaled)
plot_dual_subplot(X_umap, y_grad, filenames, speaker_ids, "UMAP (Gradient Field)", "Gradient_UMAP")
plot_dual_by_gender(X_umap, y_grad, filenames, speaker_ids, "UMAP (Gradient Field)", "Gradient_UMAP")


# ==========================================
# 5. 분석 2: Whisper Encoder
# ==========================================
...
plot_dual_subplot(X_wavlm_umap, y_wavlm, filenames_v, speaker_ids_v, "UMAP (WavLM-Large)", "WavLM_UMAP")
plot_dual_by_gender(X_wavlm_umap, y_wavlm, filenames_v, speaker_ids_v, "UMAP (WavLM-Large)", "WavLM_UMAP")

del wavlm_model, wavlm_extractor
torch.cuda.empty_cache()
gc.collect()

print(f"\n=== 분석 완료! '{FIGURE_DIR}' 폴더를 확인하세요. ===")
