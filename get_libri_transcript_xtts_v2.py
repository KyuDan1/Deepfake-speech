...
OUTPUT_DIR = Path(f"generated_results_XTTS_v2").resolve()
PROMPT_WAV_DIR = Path("prompt_wav_files").resolve()
FIGURE_DIR = Path(f"analysis_figures_xtts_v2").resolve()
...
if to_generate:
    print(f"=== XTTS-v2 모델 로딩 및 오디오 생성 ({len(to_generate)}개) ===")
    os.environ["CUDA_VISIBLE_DEVICES"] = CUDA_DEVICE
    
    # 모델 로드 (생성 시에만 로드하고 바로 해제하여 메모리 확보)
...
for item in input_data:
    ...
    if f is not None:
        features.append(f); labels.append(0); filenames.append(f"s_{item['idx']}")
        speaker_ids.append(item['speaker_id'])
    # Gen
    if os.path.exists(item['output_path']):
        f = get_gradient_features(item['output_path'])
        if f is not None:
            features.append(f); labels.append(1); filenames.append(f"s_{item['idx']}")
            speaker_ids.append(item['speaker_id'])
...
def get_wavlm_features(audio_path):
    try:
        ...
        # Last Hidden State의 시간 축 평균(Mean Pooling)
        return outputs.last_hidden_state.mean(dim=1).squeeze(0).cpu().numpy()
    except Exception as e:
        print(f"Error: {e}")
        return None
...
print("\n=== 모든 분석 완료! 'analysis_figures_xtts_v2' 폴더를 확인하세요. ===")